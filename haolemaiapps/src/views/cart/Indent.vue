<template>
    <div>
        <titlesqqq msg="确认订单"></titlesqqq>
        <div class="siteooopt">
     
     <mt-popup
  v-model="popupVisible"
  position="bottom" modal=true slot="eeeee" v-if="active==turess?0:1">
      <div class="page-picker-wrapper">
      <mt-picker :slots="addressSlots" @change="onAddressChange" :visible-item-count="5" class="page-picker"></mt-picker>
    </div>
</mt-popup>
    <div class="page-picker-desc"> <p class="picker_one"><span @click="turess=turess==active?1:0">选择地区：</span><span> {{addressProvince}}</span>
     <span> {{addressCity}}</span>   <span> {{addressArea}}</span></p>
    <p>详细地址:</p>
    </div>

        </div>
        <div class="divqq">
            <p>商品清单<span>1件</span></p>
        </div>
        <div class="conterst ">
            <div class="www">
               <div class="ww_left">
               ...
               <div class="ww_center">
                   <p>Cleef 男式 Cleef男式休闲鞋韩版低帮板鞋男鞋时尚皮鞋软面牛皮加布板鞋</p>
        ...
        <!--结束商品-->
        <p class="dis_p"><span>配送方式：</span><span>普通快递></span></p>
        <p class="dis_p"><span>优 惠 券：</span><span>无可用></span></p>
        <p class="dis_p"><span>发票信息：</span><span>不开发票></span></p>
        <p class="dis_p">订单结算</p>
         <div class="coupons">
          <p class="dis_p"><span>商品总价</span><span>￥335</span></p>
        <p class="dis_p"><span>活动优惠</span><span>-¥30</span></p>
        <p class="dis_p"><span>代金券</span><span>-¥0</span></p>
        <p class="plist_last">运费总额
                                    <span style="margin-left: 0.3rem;font-size: 0.65rem;color: #999;">会员年度消费满2000元，单笔订单满99元可享受免运费服务（全球购商品除外）。您已消费<em>0</em>元</span>
                                <span class="moneynum" id="deliveryPrice">+¥<i>10</i></span>
                </p>
        </div>

        <mt-button size="large" class="aae">微信支付</mt-button>
         <mt-button size="large" type="primary">支付宝支付</mt-button>
          <mt-button type="danger" size="large">银行卡支付</mt-button>
    </div>
</template>
...
import title_bag from '../../components/TitleBack'
...
export default {
    ...
    methods: {
        ...
        // picker.setSlotValues(1, address[0]);
...
<style lang="scss">
.moneynum{text-align: right;float: right;}
.plist_last{padding: .6rem;line-height:2em}
.coupons{
    width:100%;
    p{border: 0;color:#666;font-size: .6rem;}
}
.dis_p{width:100%;display: flex;justify-content:space-between;padding:.6rem;box-sizing: border-box;
   border-bottom: 1px solid #d7d7d7;
}
.conterst{width:100%;
 .www{width:100%;height:6rem;display: flex;align-items: center;padding-left: .6rem;box-sizing:border-box;border-top:1px solid #d7d7d7;
   .ww_left{width:20%;height: 64px;border:1px solid #d7d7d7;display: flex;justify-content: center;align-items: center;}
   .ww_right{width:20%;align-self:flex-start;text-align: right;padding: 1rem;
   }
   .ww_center{width:60%;padding:1rem;font-size: .2rem;
    p:first-child{padding-bottom: .6rem;}
    p:nth-child(2){
...
.divqq{width:100%;height: 2rem;padding-left: .6rem;
 p{padding: .3rem .3rem;}
}
.picker_one{width:100%;height: 2rem;}
.siteooopt{margin-bottom: 1rem;}
.page-picker-desc{width:100%;height: 7rem;line-height: 4rem; position: relative;
    top: 3.25rem;
background: url('../../img/orderaddressbg.jpg') no-repeat;
background-size: 100% 7rem;
}
.slot2{width:20%;}
.aae{background-color: #4AD52F!important;color:white!important;}
.page-picker{width:375px!important;background-color: #FFFFFF!important;}
 .picker-items{width:100%!important;}
   .detailed-address{width:100%;height: 4rem;border-bottom:2px solid #000;}
    .page-picker-wrapper{
